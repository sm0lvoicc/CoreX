...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        `${emoji.voice} Total Voice Channels:`,
        message.guild.channels.cache.filter(
      ...
      .addField(
        `${emoji.mention} Total Amount of Roles:`,
        message.guild.roles.cache.size,
        true,
      )
      .addField(
        `${emoji.success} Verification Level`,
      ...
      .addField(
        `${emoji.star} Roles [${roles.length}]`,
        roles.length < 15
          ? roles.join(", ")
          : roles.length > 15
          ? `${roles.slice(0, 15).join(", ")}\n+${roles.length - 15} roles...`
          : "None",
      )
