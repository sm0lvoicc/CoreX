    ...
    if(query.toLowerCase() == "add") {
      ...
      schema.findOne(guild, async(err,data) => {
        ...
        if(data) {
          ...
          if(data.Member.includes(word.id)) return message.channel.send(`${emoji.error} **${word.user.tag}** is already in the Anti-ping system`)
          data.Member.push(word.id)
          data.save()
          const embed = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(`${emoji.success} **${word.user.tag}** has been added to the Anti-Ping system`)
      ...
      schema.findOne(guild, async(err,data) => {
        if(!data) return message.channel.send(`${emoji.error} There is no member in Anti-Ping`)
        if(!data.Member.includes(word.id)) return message.channel.send(`${emoji.error} **${word.user.tag}** isn't in the Anti-Ping system`)
        ...
        const embed = new MessageEmbed()
        .setColor('GREEN')
        .setDescription(`${emoji.success} **${word.user.tag}** has been removed from the Anti-Ping system`)
        message.channel.send(embed)
      })
    } else if(query.toLowerCase() == 'display') {
      schema.findOne(guild, async(err, data) => {
        if(!data) return message.channel.send(`${emoji.error} There is no member in the Anti-Ping system`);
