...
module.exports = {
        ...
        if(opt === 'set') {
            ...
            schema.findOne({ Guild: message.guild.id }, async(err, data) => {
                if(!data) {
                    ...
                    .setColor('GREEN')
                    .setDescription(`${emoji.success} Message-logs is set to ${channel}`)
    ...
    if(opt === 'disable') {
        schema.findOne({ Guild: message.guild.id}, async(err, data) => {
            if(!data) message.channel.send(`${emoji.error} The Message-logs is already disabled`)
            data.delete()
            message.channel.send(`${emoji.success} Message logging has been disabled`)
        })
    }
