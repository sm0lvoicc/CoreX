...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (opt === "enable") {
      ...
      if (!["warn", "mute", "kick", "ban"].includes(action.toLowerCase()))
        return message.channel.send(
          new MessageEmbed()
            .setColor("RED")
      ...
      if (messages <= 2)
        return message.channel.send(
          "Messages cannot be less than or equal to 2 messages",
        );
    ...
    if (opt === "disable") {
      schema.findOne({ Guild: message.guild.id }, async (err, data) => {
        if (!data)
          return message.channel.send(
            `${emoji.error} Anti-Spam module is already disabled`,
          );
        data.delete();
        message.channel.send(
          `${emoji.success} Anti-Spam module has been disabled`,
        );
      });
    }
  },
