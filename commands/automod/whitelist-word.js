    ...
    run: async(client, message, args) => {
        ...
        if(opt == 'add') {
            ...
            schema.findOne({ Guild: message.guild.id}, async(err, data) => {
                if(!data) {
                    ...
                    newData.save()
                    message.channel.send(`${emoji.success} Whitelisted Bad Words in ${channel}`)
                } else {
                    if(data.Anti_curse.includes(channel.id)) return message.reply(`${emoji.error} This channel is already whitelisted`)
        ...
        if(opt == 'remove') {
            ...
            schema.findOne({ Guild: message.guild.id}, async(err, data) => {
                ...
                if(!data.Anti_curse.includes(channel.id)) return message.channel.send(`${emoji.error} ${channel} is not whitelisted`)
