...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(opt == 'add') {
            ...
            schema.findOne({ Guild: message.guild.id}, async(err, data) => {
                if(!data) {
                    ...
                    if(data.Anti_invite.includes(channel.id)) return message.reply('<:corexerror:860580531825147994> This channel is already whitelisted')
        ...
        if(opt == 'remove') {
            ...
            schema.findOne({ Guild: message.guild.id}, async(err, data) => {
                if(!data) return message.reply('<:corexwarn:860597628882780200> There are no channels whitelisted')
                if(!data.Anti_invite.includes(channel.id)) return message.reply(`<:corexerror:860580531825147994> ${channel} is not whitelisted`)
