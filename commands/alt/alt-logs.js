...
module.exports = {
  ...
  timeout: 5000,
  aliases: ["set-altlogs", "set-alt-logs", "set-alt-logs"],
  usage: "<set/disable>",
    ...
    if (opt === "set") {
      ...
      if (!channel)
        return message.channel.send(
          "Please mention a channel to set as the alt logs",
        );
      ...
      schema.findOne({ Guild: message.guild.id }, async (err, data) => {
        if (!data) {
          ...
          const embed2 = new MessageEmbed()
            .setColor("GREEN")
            .setDescription(
              `${emoji.success} Alt-logs is updated to  ${channel}`,
            )
            .setTimestamp();
      ...
      schema.findOne({ Guild: message.guild.id }, async (err, data) => {
        if (!data)
          message.channel.send(
            `${emoji.error} The Alt-logs is already disabled`,
          );
