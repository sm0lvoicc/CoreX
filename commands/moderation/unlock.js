...
module.exports = {
    ...
    run: async(client, message, args) => {
       ...
       if(!channel) return message.channel.send('Mention a channel to unlock')
        ...
        if(channel.permissionsFor(everyone).has(["SEND_MESSAGES"])) return message.channel.send(`${emoji.error} This channel is already unlocked.`)
        ...
        channel.send(new MessageEmbed().setDescription(`${emoji.lock} **${message.author.tag}** unlocked this channel`).setColor("GREEN"))
        .catch(err => {
