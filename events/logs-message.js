...
client.on("messageDelete", async (message) => {
  ...
  db.findOne({ Guild: message.guild.id }, async (err, data) => {
    ...
    if (message.content.length > 2000)
      banReason = banReason.slice(0, 1997) + "...";
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
  ...
  db.findOne({ Guild: newMessage.guild.id }, async (err, data) => {
    ...
    const messageUpdateEmbed = new MessageEmbed()
      ...
      .addField(
        "Edited in",
        `${newMessage.channel} | [Jump to message](https://discordapp.com/channels/${newMessage.guild.id}/${newMessage.channel.id}/${newMessage.id})`,
      )
